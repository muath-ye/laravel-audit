...
namespace Muathye\Audit\Storage;
...
class DbStorage implements StorageInterface
{
    ...
    protected $sqlQueries = array(
        'save' => "INSERT INTO %tablename% (id, data) VALUES (?, ?)",
        'get' => "SELECT data FROM %tablename% WHERE id = ?",
        'find' => "SELECT data FROM %tablename% %where% ORDER BY id DESC LIMIT %limit% OFFSET %offset%",
     ...
     */
    public function __construct(PDO $pdo, $tableName = 'muathye_audit', array $sqlQueries = array())
    {
        $this->pdo = $pdo;
    ...
    public function save($id, $data)
    {
        ...
        $meta = $data['__meta'];
        $stmt->execute(array($id, serialize($data)));
    }
